...
const getPriceFromNumber = (number: number) =>
  Math.abs(Math.tan(number + 1)) * 1000;

const getItemFromNumber = (number: number) => items[number % items.length];

export const getMockedItems = (amount: number) =>
  range(0, amount).map((number) => {
    const itemMock = getItemFromNumber(number);
    const price = getPriceFromNumber(number);
